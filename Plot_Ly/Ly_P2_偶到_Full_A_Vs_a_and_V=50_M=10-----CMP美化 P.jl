...
function get_value(M,method,V,arrive)
    df = collect_results(datadir("Lytemp10Cmp/$(method)_$(V)_$(M)_$(arrive)"))
    return mean(df.average_P )#mean(df.average_H)
end
values = []
Q_values = []
P_values = []
...
data = hcat(data_arrive2,data_arrive3, data_arrive4, data_arrive5, data_arrive6)#, data_arrive10, data_arrive12,data_arrive14,data_arrive16)
Marker_Size = 2
Line_Width=2
...
plot(xs, data[:, 1], color="b",  linestyle="--",  marker="o", label="M-QH算法",linewidth = Line_Width)
plot(xs, data[:, 2],color="r",  linestyle="--", marker="s", label="星上贪婪",linewidth = Line_Width)
plot(xs, data[:, 3],color="g",  linestyle="--", marker="^", label="公平调度",linewidth = Line_Width)
plot(xs, data[:, 4], color="orange", linestyle="--", marker="D", label="随机调度",linewidth = Line_Width)
plot(xs, data[:, 5], color="black", linestyle="--", marker="v", label="\$\\epsilon-\$年龄贪婪",linewidth = Line_Width)
...
xlabel("任务到达速率\$\\lambda\$(个/每秒)", fontsize = 8)
...
legend(fontsize = 8)
# size(500,100)
savefig(plotsdir("Ly_final_比较图8.png"), bbox_inches = "tight", dpi=1200)
