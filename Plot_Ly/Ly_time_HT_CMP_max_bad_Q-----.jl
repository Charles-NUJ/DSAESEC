...
plot(xs, data_arrive1[1:3500], color="red",  linestyle="-", label ="M-R",linewidth = Line_Width)
plot(xs, data_arrive2[1:3500],color="blue",  linestyle="-",  label ="M-Q", linewidth = Line_Width)
plot(xs, data_arrive3[1:3500],color="black",  linestyle="-", label ="M-H",linewidth = Line_Width)
plot(xs, data_arrive4[1:3500], color="green", linestyle="-", label ="M-QH",linewidth = Line_Width)
...
savefig(plotsdir("Ly_time_CMP_Q-QH_max_bad_Q图3.png"), bbox_inches = "tight", dpi=1200)
# end
end
# Ms = [5]#, 10, 15, 20, 25]
plot_run()