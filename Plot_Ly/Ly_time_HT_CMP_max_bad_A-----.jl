...
plot(xs, data_arrive1[1:T], color="red",  linestyle="-", label ="M-R",linewidth = Line_Width)
plot(xs, data_arrive2[1:T],color="blue",  linestyle="-",  label ="M-Q", linewidth = Line_Width)
plot(xs, data_arrive3[1:T],color="black",  linestyle="-", label ="M-H",linewidth = Line_Width)
plot(xs, data_arrive4[1:T], color="green", linestyle="-", label ="M-QH",linewidth = Line_Width)
...
savefig(plotsdir("Ly_time_CMP_Q-QH_max_bad_A图5.png"), bbox_inches = "tight", dpi=1200)
# end
end
# Ms = [5]#, 10, 15, 20, 25]
plot_run()