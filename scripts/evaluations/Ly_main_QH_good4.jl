...
function run_experiment(graph_size, config)
	...
	wsave(datadir("Lytemp5/$(Method)_$(V)_$(M)_$(arrive)", savename(config, "jld2")), result)
end
			...
			for arrive in arrives
				for graph_size in graph_sizes
					...
					for r in res#[1:50] 
						...
						if !isfile(datadir("Lytemp5/$(Method)_$(V)_$(M)_$(arrive)", savename(config, "jld2"))) || force_calculation
							println("Running Ly QH 25 $config")
							# println("$(Dates.now())")
...
arrives = [2,4,6,8]
graph_sizes=[5,10,15,20,25]
Vs = [0.1, 0.2, 0.25, 0.5, 1, 5,10, 25, 50, 100, 200,300, 500.0]
