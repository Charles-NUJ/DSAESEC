...
function run_experiment(graph_size, config)
	...
	@inbounds for t in 1:T
		...
		# node_index_temp = mod(t,M) # 轮流调度
		# node_index=1
		# if node_index_temp == 0
		# 	node_index = M
		# else
		...
		if random_value >= 0.7
			...
			if length(pool_index)>0
				node_index_index = rand(1:length(pool_index))
				node_index = pool_index[node_index_index]
			end

		end

		this_Q_length = Q[node_index]
        cpu_index=8
		#前任务最优时钟频率
		flage = 0
		for cpu_index_temp in 1:8
			if this_Q_length - (CPU_S[cpu_index_temp]) ./ gamma <=0 && flage==0
				flage=1
			end
			if flage==1
				cpu_index = cpu_index_temp
				break
			end
		end
	...
	wsave(datadir("Lytemp10Cmp/$(Method)_$(V)_$(M)_$(arrive)", savename(config, "jld2")), result)
end
			...
			for arrive in arrives
				for graph_size in graph_sizes
					...
					for r in res#[1:50] 
						...
						if !isfile(datadir("Lytemp10Cmp/$(Method)_$(V)_$(M)_$(arrive)", savename(config, "jld2"))) || force_calculation
							println("Running Ly epsilonAoIGreedy 10 $config")
							# println("$(Dates.now())")
